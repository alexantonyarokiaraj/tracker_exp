          ...
          try:
            ...
            if batch_mode:
                if save_canvas:
                    ...
                    # Semantic colors: beam=grey, each scatter cluster gets a distinct color
                    _beam_color = root.kGray + 1
                    _scatter_palette = [root.kRed, root.kBlue, root.kGreen + 2,
                                        root.kOrange + 1, root.kMagenta, root.kCyan + 1]

                    def _make_type_colors(col_idx, type_col_idx):
                        """Map cluster labels to colors: beam->grey, scatter N -> palette[N]."""
                        _result = {}
                        _scat_idx = 0
                        for _lbl in sorted(np.unique(data_points_3d[:, col_idx])):
                            if _lbl in (-1, -20):
                                continue
                            _is_scatter = int(
                                data_points_3d[data_points_3d[:, col_idx] == _lbl, type_col_idx].mean() + 0.5
                            ) == 1
                            if _is_scatter:
                                _result[_lbl] = _scatter_palette[_scat_idx % len(_scatter_palette)]
                                _scat_idx += 1
                            else:
                                _result[_lbl] = _beam_color
                        return _result

                    _gmm_type_colors = _make_type_colors(
                        DataArray.GMM.value, DataArray.REGULARIZED_TRACK_TYPE.value)
                    _reg_type_colors = _make_type_colors(
                        DataArray.REGULARIZED_CDIST.value, DataArray.REGULARIZED_TRACK_TYPE.value)

                    # Row 2: GMM labels (XY, YZ, XZ)
                    graphs_gmm = plot_3d_projections(data_points_3d, DataArray.GMM, c1, [4, 5, 6], filter_label=filter_label, fixed_colors=_gmm_type_colors)

                    # Row 3: Regularized cdist-merged labels (XY, YZ, XZ)
                    graphs_reg = plot_3d_projections(data_points_3d, DataArray.REGULARIZED_CDIST, c1, [7, 8, 9], filter_label=filter_label, fixed_colors=_reg_type_colors)
                ...
                if save_canvas:
                    _ransac_type_colors = _make_type_colors(
                        DataArray.RANSAC_CDIST.value, DataArray.RANSAC_TRACK_TYPE.value)
                    graphs_ransac = plot_3d_projections(data_points_3d, DataArray.RANSAC_CDIST, c1, [10, 11, 12], filter_label=filter_label, fixed_colors=_ransac_type_colors)
                ...
                for method_tag, endpoints_list, lbl_col, type_col in [
                    ...
                    for grp_idx, group_eps in enumerate(groups.values()):
                        ...
                        colors_grp = [_zoom_scat_palette[i % len(_zoom_scat_palette)] for i in range(mult)]
                        beam_pixel_color = root.kGray + 2  # distinct from all scatter palette entries
                        ...
                        # ── Pass 1a: draw beam pixel boxes first (scatter drawn on top in 1b) ─
                        ...
                        for ep_i, ep in enumerate(group_eps):
                            ...
                            # Raw scattered-track data points (filled pixel boxes, drawn on top of beam)
                            track_mask = (
                                (data_points_3d[:, lbl_col].astype(int) == ep.track_id) &
                                (data_points_3d[:, type_col].astype(int) == 1)
                            )
